    ...
    F: FnMut(Separated<'_, 'args, Sqlite, &'static str>, I::Item),
{
    ...
    for chunk in tuples
        .into_iter()
        .chunks(const { SQLITE_LIMIT / N })
        .into_iter()
        // FIXME: find a way to not collect intermediate vectors
        .map(|chunk| chunk.collect::<Vec<_>>())
        // need to collect again because Itertool's IntoChunks is not Send/Sync, no good for async
        .collect::<Vec<_>>()
    {
...
pub async fn scan_into(client: &Client, conn: &mut SqliteConnection) -> Result<()> {
    ...
    // FIXME: load data lazily, by chunks; make use of query batches
